...
void CSettingGitRemote::OnLbnSelchangeListRemote()
{
	...
	m_strUrl.Empty();
	m_strUrl = g_Git.GetConfigValue(cmd,CP_GIT_XUTF8);

	cmd.Format(_T("remote.%s.puttykeyfile"),remote);

	this->m_strPuttyKeyfile=g_Git.GetConfigValue(cmd,CP_GIT_XUTF8);
...
void CSettingGitRemote::Save(CString key,CString value)
{
	...
	if(g_Git.SetConfigValue(cmd, value, CONFIG_LOCAL, CP_GIT_XUTF8, &g_Git.m_CurrentDir))
	{
