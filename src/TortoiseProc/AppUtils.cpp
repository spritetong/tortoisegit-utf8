...
int	 CAppUtils::StashApply(CString ref, bool showChanges /* true */)
{
	...
	int ret = g_Git.Run(cmd, &out, CP_GIT_XUTF8);
...
bool CAppUtils::ConflictEdit(CTGitPath &path,bool /*bAlternativeTool*/,bool revertTheirMy)
{
	...
	for(int i=0;i<list.GetCount();i++)
	{
		...
		CString output, err;
		if(!outfile.IsEmpty())
			if (!g_Git.Run(cmd, &output, &err, CP_GIT_XUTF8))
			{
				CString file;
				int start =0 ;
...
int CAppUtils::GetCommitTemplate(CString &temp)
{
	...
	output= g_Git.GetConfigValue(_T("commit.template"),CP_GIT_XUTF8);
	if( output.IsEmpty() )
		return -1;
...
void CAppUtils::EditNote(GitRev *rev)
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		try
		{
			if(git_run_cmd("notes", CUnicodeUtils::GetMulti(cmd,CP_GIT_XUTF8).GetBuffer()))
			{
...
int CAppUtils::GetMsysgitVersion(CString *versionstr)
{
	...
	else
	{
		...
		if(g_Git.Run(cmd, &version, &err, CP_GIT_XUTF8))
		{
