...
int CPatch::SendPatchesCombined(CTGitPathList &list,CString &To,CString &CC, CString &subject,bool bAttachment, bool useMAPI,CString *errortext)
{
	...
	for(int i=0;i<list.GetCount();i++)
	{
		...
		{
			g_Git.StringAppend(&body,(BYTE*)patch.m_Body.GetBuffer(),CP_GIT_XUTF8,patch.m_Body.GetLength());
		}
...
int CPatch::Parser(CString &pathfile)
{
	...
	one=m_Body.Tokenize("\n",start);
	if(one.GetLength()>6)
		g_Git.StringAppend(&m_Author,(BYTE*)one.GetBuffer()+6,CP_GIT_XUTF8,one.GetLength()-6);

	one=m_Body.Tokenize("\n",start);
	if(one.GetLength()>6)
		g_Git.StringAppend(&m_Date,(BYTE*)one.GetBuffer()+6,CP_GIT_XUTF8,one.GetLength()-6);

	one=m_Body.Tokenize("\n",start);
	if(one.GetLength()>9)
		g_Git.StringAppend(&m_Subject,(BYTE*)one.GetBuffer()+9,CP_GIT_XUTF8,one.GetLength()-9);

	//one=m_Body.Tokenize("\n",start);

	g_Git.StringAppend(&m_strBody,(BYTE*)m_Body.GetBuffer()+start+1,CP_GIT_XUTF8,m_Body.GetLength()-start-1);
